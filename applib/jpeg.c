...
// ---- ブロックのデコード ---
// ハフマンデコード＋逆量子化＋逆ジグザグ
int jpeg_decode_huff(JPEG *jpeg,int scan,int *block, unsigned char *zigzag_table)
{
...
// MCU一個変換
int jpeg_decode_mcu(JPEG *jpeg, unsigned char *zigzag_table)
{
	int scan, val;
	int unit, i, h, v;
	int *p, hh, vv;
